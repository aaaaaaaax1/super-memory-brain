...
if ($repairHookText -like '*load Skill super-memory-brain first*' -and $repairHookText -like '*bare*' -and $repairHookText -like '*optimize/start/refresh*' -and $repairHookText -like '*memory:auto*' -and $repairHookText -like '*semantic/keyword recall*') { Mark-Ok 'repair hook short router rule' } else { Mark-Fail 'repair hook short router rule missing' }
...
if ($skillText.Contains('Bare Super Brain trigger rule') -and $skillText.Contains('Super Brain fault trigger rule') -and $skillText.Contains('Do not trigger this rule for clear human self-reports') -and $skillText.Contains('Do not treat these bare wake words as ordinary greeting/chat') -and $skillText.Contains('Explicit skill links') -and $skillText.Contains('`超级大脑`') -and $skillText.Contains('`G1`')) { Mark-Ok 'entry skill bare Super Brain trigger rule' } else { Mark-Fail 'entry skill bare Super Brain trigger rule missing' }
...
if ($skillText.Contains('memory:auto') -and $skillText.Contains('keyword + semantic triggers') -and $skillText.Contains('Plan/Explore/Tool thresholds')) { Mark-Ok 'entry skill short memory router policy' } else { Mark-Fail 'entry skill short memory router policy missing' }
...
if ($skillText.Contains('final visible answer') -and $skillText.Contains('standalone `G1` line') -and $skillText.Contains('Do not add `G1` to interim progress updates') -and $skillText.Contains('Prevent logic breakpoints') -and $skillText.Contains('continue from the next concrete action')) { Mark-Ok 'entry skill final G1 prefix and breakpoint recovery rules' } else { Mark-Fail 'entry skill final G1 prefix or breakpoint recovery rules missing' }
...
if ($skillText.Contains('Commander Team Memory') -and $skillText.Contains('off the cold-start path') -and $skillText.Contains('ordinary startup, simple `继续`') -and $skillText.Contains('Trigger Commander Team Memory only') -and $skillText.Contains('Findings without evidence are assumptions')) { Mark-Ok 'entry skill Commander team cold-start gate' } else { Mark-Fail 'entry skill Commander team cold-start gate missing' }
...
if ($orcText.Contains('Team Dispatch On Demand') -and $orcText.Contains('routing dormant by default') -and $orcText.Contains('Do not run dispatch scoring') -and $orcText.Contains('review_board') -and $orcText.Contains('Code-capable subagents require explicit Commander authorization')) { Mark-Ok 'ORC team dispatch on-demand rules' } else { Mark-Fail 'ORC team dispatch on-demand rules missing' }
...
if ($startupCheckText -like '*Hook short router*' -and $startupCheckText -like '*semantic/keyword recall*' -and $startupCheckText -like '*memory:auto*') { Mark-Ok 'startup hook short router check' } else { Mark-Fail 'startup hook short router check missing' }
if ($startupCheckText -like '*Hook bare Super Brain wake words*' -and $startupCheckText -like '*load Skill super-memory-brain first*' -and (($startupCheckText -like '*超级大脑*') -or ($startupCheckText -like '*0x8D85*')) -and $startupCheckText -like '*G1*') { Mark-Ok 'startup hook bare wake word check' } else { Mark-Fail 'startup hook bare wake word check missing' }
...
if ($triggerSimulationText -like '*bare_superbrain_zh*' -and $triggerSimulationText -like '*bare_g1*' -and $triggerSimulationText -like '*superbrain_optimize*' -and $triggerSimulationText -like '*expectedSkill*' -and $triggerSimulationText -like '*requiresG1*' -and $triggerSimulationText -like '*bare_superbrain_wake_word*') { Mark-Ok 'trigger simulation bare Super Brain skill coverage' } else { Mark-Fail 'trigger simulation bare Super Brain skill coverage missing' }
if ($triggerSimulationText -like '*brain_fault_bare*' -and $triggerSimulationText -like '*superbrain_fault_semantic_trigger*' -and $triggerSimulationText -like '*human_brain_self_report*' -and $triggerSimulationText -like '*skill-negative*' -and $triggerSimulationText -like '*human_self_report_excluded*') { Mark-Ok 'trigger simulation Super Brain fault exclusion coverage' } else { Mark-Fail 'trigger simulation Super Brain fault exclusion coverage missing' }
