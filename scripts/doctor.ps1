...
function Run-JsonScript([string]$Name) {
  ...
  switch ($Name) {
    'summary.ps1' { $output = & $path -Json }
    'startup-check.ps1' { $output = & $path -Json }
    'skill-sync-check.ps1' { $output = & $path -Json }
    'memory-health.ps1' { $output = & $path -Json }
    'script-tiers.ps1' { $output = & $path -Json }
    default { $output = & $path }
  }
  ...
  return [pscustomobject]@{ name=$Name; ok=($exitCode -eq 0); exitCode=$exitCode; data=$parsed }
}
...
foreach ($item in @($summary,$startup,$skillSync,$memoryHealth,$scriptTiers)) {
...
$result = [pscustomobject]@{
  ...
  checks = @(@($summary,$startup,$skillSync,$memoryHealth,$scriptTiers) | ForEach-Object { [pscustomobject]@{ name=$_.name; ok=$_.ok; exitCode=$_.exitCode } })
}
