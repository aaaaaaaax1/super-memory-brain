...
它会转发到 `scripts\bootstrap.ps1`，按固定顺序完成技能注入、正式记忆根初始化、hook 安装、MCP 注册、首载自检、编码/图谱维护和集成验证。重复运行是幂等的，并保留安装备份。朋友安装请先阅读 `FRIEND_INSTALL.md`。
...
The install flow also installs the local Python runtime and registers the narrow
`super-memory-brain` MCP. The first conversation after installation should be a
new Codex task so the three read-only MCP tools are discovered. When an explicit
Super Brain request is loaded later, the UserPromptSubmit hook performs a
first-load check and repairs a missing or stale MCP binding against the formal
memory root before reporting readiness.

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "<package-root>\scripts\runtime-status.ps1" -Json
powershell -NoProfile -ExecutionPolicy Bypass -File "<package-root>\scripts\runtime-eval.ps1" -McpReplay -Json
```

For a fresh empty memory root, installation runs the runtime/MCP contract check
without treating the absence of historical memory as a recall failure. A
populated memory root runs the full twelve-case recall regression.
