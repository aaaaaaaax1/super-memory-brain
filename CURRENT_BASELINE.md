...
- [VERIFIED] Goal Route Lock is active: `scripts/goal-route-lock.ps1` and `scripts/route-checkpoint.ps1` preserve accepted goals/routes/non-goals and emit `ROUTE_DRIFT_DETECTED` for scope creep, unapproved goal change, or route drift.
- [VERIFIED] Verified Integration Guard is active: `scripts/verified-module-snapshot.ps1` records verified module contracts and `scripts/integration-parity-check.ps1` reports `INTEGRATION_DRIFT_DETECTED` when entrypoint, environment, dependencies, state, scattered assembly, or acceptance-path evidence breaks parity.
- [VERIFIED] Acceptance evidence is layered: `module smoke OK`, `integration smoke OK`, and `user-facing acceptance OK` are separate fields in integration/task verification instead of being collapsed into one claim.
- [VERIFIED] Self-learning candidates now include `gap` and `logic_breakpoint` for missing route locks, missing integration parity, missing acceptance paths, stale state, module context changes, scattered assembly, and false completion risk.
- [VERIFIED] Structural change reasoning frame is active in cognitive preflight and `scripts/causal-change-plan.ps1`: known facts -> causal mechanism -> intended change -> expected optimization -> verification evidence -> residual risk, plus explicit observed problem, root cause, prior changes, verification method, and residual risk fields.
- [VERIFIED] Cognitive enforcement gate is active: `scripts/cognitive-enforce.ps1` writes `last-cognitive-enforce.json` and blocks high-risk work when fresh cognitive preflight, `mustPreserve`, or `driftGuards` are missing.
- [VERIFIED] Runtime drift checkpointing is active: `scripts/runtime-drift-checkpoint.ps1` writes `runtime-drift-checkpoint.json` / `last-runtime-drift-checkpoint.json`, reports `DRIFT_DETECTED`, tracks `unresolvedDrift`, and protects BeforeAct/BeforeMutation/BeforeCompletion/AfterUserCorrection phases.
- [VERIFIED] Governed self-learning loop is active: `scripts/reflection-promotion.ps1` generates learning candidates from corrections, failures, verification, retrospectives, and drift state; Analyze/Preview does not write durable memory, while Apply routes through governed `learn-memory.ps1`, `write-experience.ps1`, or `skill-evolution.ps1`.
- [VERIFIED] AgentBridge procedure memory card is active: `memory/workspace/procedure-cards/agent-bridge-channel.json` stores triggers, must-do, must-not-do, drift guards, step flow, verification checklist, and related failures for reuse by cognitive preflight.
- [VERIFIED] Cognitive execution preflight is active: `scripts/cognitive-preflight.ps1` turns Super Brain memory into an execution-control card with user hard rules, accepted constraints, similar experiences, AgentBridge/domain reflexes, `mustPreserve`, and `driftGuards` before action.
- [VERIFIED] Memory is layered as cognition, not only storage: semantic memory keeps stable facts/preferences/project knowledge, episodic memory keeps prior task traces/failures, procedural memory keeps reusable workflows/checklists, and working memory keeps current goal/constraints/evidence/assumptions/next action.
- [VERIFIED] Drift control is active: tasks matching prior failures must stop on `DRIFT_DETECTED`, return to accepted constraints, and only then continue; live files/tool results override stale memory and significant user corrections trigger reflection/lesson promotion.
- [VERIFIED] Agent Bridge no-nested-agent target command is active: `开启子agent通道` means the current Codex/ZCode target conversation opens a local channel with `agent-bridge-channel.ps1 -Action Open`; it must not create a nested helper/worker/explorer/Tesla agent to open the channel.
- [VERIFIED] Agent Bridge target-mode idle wait semantics are active: `WaitConnect` / `WaitInbox` no-message/no-connection waits return `idle_waiting_connect` / `idle_waiting_message` with `notBlocked` and `noProgressReportRequired`, so the sub-agent must not mark the goal blocked/paused/failed/completed or emit repeated status messages.
- [VERIFIED] Agent Bridge channel open is fresh by default: `开启子agent通道` / `Open` creates a new channel for the current sub-agent session unless the user explicitly supplies a channel id, so it does not accidentally reuse an old active/last channel.
- [VERIFIED] Agent Bridge target-mode reply persistence is active: after a sub-agent receives one message and sends one reply, that reply is not `Goal completed` / `目标完成`; the sub-agent must stay in target mode and continue waiting for the next inbox message until explicit close.
- [VERIFIED] Global startup hard guard routes mixed CJK/non-English phrases containing ASCII `agent` to Super Memory Brain Agent Bridge instead of host default explorer/worker/default agent role help, unless the user explicitly asks for role help.
- [VERIFIED] Global startup bootstrap routes Agent Bridge channel phrases to Super Memory Brain first: new ZCode/Codex conversations should treat `开启agent通道`, `开启子agent通道`, `连接子agent通道`, `agent通道`, `agent bridge`, and `subagent channel` as Super Brain / agent-bridge triggers instead of host default agent role help.
- [VERIFIED] Agent Bridge natural-language short-command protocol is active: `开启子agent通道` maps to Open -> WaitConnect -> WaitInbox and treats Open success as a persistent target-mode wait state, not completion; `连接子agent通道`, `向<alias>发送信息：...`, `读取<alias>通道回复`, and explicit `关闭<alias>通道` route to Connect, SendAndWait, WaitReply, and Close respectively.
- [VERIFIED] Agent Bridge subordinate wait-state workflow is active: controlled/sub-agent sessions `Open` once with `waiting_connect`, use `WaitConnect` for one connected notification, then use `WaitInbox` for bounded silent message waiting with no repeated waiting output and no auto-close unless the user/main agent explicitly closes the channel.
- [VERIFIED] Agent Bridge target-mode active channel workflow is active: controlled/sub-agent sessions can `Open` first, the main/operator agent can `Connect` once with an alias such as `子agent`, `active-agent-bridge-channel.json` remembers the channel until user close, and `SendAndWait` sends through the remembered channel with bounded reply waiting.
- [VERIFIED] Agent Bridge shared channel conversations are active: `agent-bridge-channel.ps1` supports Open/Send/Inbox/Ack/Close/Status with `target-session` routing, per-channel files under `memory/workspace/agent-bridge/channels`, compact same-channel replies, and advisory-only state unless Commander adopts a result.
- [VERIFIED] Lightweight task registration fast path is active: `task-register.ps1` writes shared agent/session/task/link identity cards for Codex/ZCode status broadcast without touching `active-checkpoint.json` or running doctor, verify-package, hot-refresh, CI, dashboard, auto-check, or recall.
- [VERIFIED] Cross-agent/session task identity indexing is active: `checkpoint-writer.ps1` writes shared agent/session/task cards with `agentId`, `sessionId`, `sessionName`, `taskId`, `taskName`, and `memoryIds`; `task-index.ps1` reads shared task cards plus legacy checkpoints, supports `-Agent`, `-SessionId`, `-IncludeCompleted`, and `-Table`, and reports unregistered sessions as `未知，不等于没有任务`.
- [VERIFIED] Agent Bridge dispatch/resilience extras are active: `agent-bridge-dispatch.ps1` generates target-agent/new-session handoff packets, `agent-bridge-permissions.ps1` enforces advisory/commander roles, `agent-bridge-queue.ps1` provides lightweight non-real-time relay, `team-task-archive.ps1` dry-runs old team-task archival, and `host-cache-check.ps1` surfaces current-session cache limitations.
- [VERIFIED] Agent Bridge isolation is active: `agent-bridge` and `scripts/agent-bridge.ps1` support controlled multi-agent dialogue, task handoff, heartbeat, failover to another agent/new conversation, Commander-only adoption, and isolated state under `memory/workspace/agent-bridge/` without writing shared memory by default.
- [VERIFIED] Reliable task execution upgrades are active: `task-index.ps1` lists current/paused/completed/candidate tasks; `intent-gate.ps1` scripts plan/status/execute/clarify permission decisions; `recovery-e2e.ps1` validates recovery behavior; `step-ledger.ps1` records detailed step evidence/verification/blockers; task-index ranks resume candidates; `host-cache-check.ps1` detects stale installed skill copies and current-session cache limitations.
- [VERIFIED] Current-task status and paste-free resume rules are active: plain `任务状态` means current conversation task status first, recovery reads visible context plus compact anchors such as active checkpoint, step ledger, status card, recent verification, and recent modified files instead of demanding long pasted history, ambiguous recovery asks for a numbered task choice, and ZCode sessions avoid TodoWrite unless explicitly requested.
...
- [VERIFIED] Default session restore omits evidence `snippet` tails, limits restore evidence cards to 2, and projects active checkpoints into short fields; `-Deep` keeps detailed snippets available when explicitly needed, while keyword/semantic triggers and Hybrid Recall remain active.
- [VERIFIED] Temporary session binding is available through `scripts/session-binding.ps1` and `memory\workspace\session-binding.json`; it stores compact evidence bundles only, respects `memory:off`, TTL, package-version and memory-root guards, and does not pollute durable shared memory.
- [VERIFIED] Session restore, Hybrid Recall, status, and doctor diagnostics expose session binding state, ignore expired or mismatched bindings, and surface preventable binding risks.
- [VERIFIED] Avoidable Issue Elimination Rule is active: preventable problems across requirement, design, implementation, verification, release, and hot-refresh stages must be resolved or guarded before completion.
- [VERIFIED] Long-memory risks discovered by status/optimization/doctor are proactively compressed into compact stable summaries while preserving key facts, anchors, decisions, scope, and graph/ADR evidence.
- [VERIFIED] Substantial features, long projects, repeated repairs, releases, and verified workflow improvements trigger proactive experience learning for reusable methods and user preference patterns.
- [VERIFIED] Compression recovery guards prevent context-compression continuation from trusting stale task summaries: `auto-continuation.ps1` marks old `last-task-verification.json` stale on version/time mismatch, while `session-restore.ps1` restores compact `status-card.json` state first.
- [VERIFIED] Project Graph Continuity anchors long-running and structure-affecting work with `memory\workspace\project-graph.json`, `task-graph.json`, `structure-baseline.json`, `step-ledger.json`, `agent-findings\*.json`, `codegraph-index.json`, `last-impact-advisor.json`, `last-completed-task-graph.json`, and `task-archive\*.json`, so current route decisions preserve critical files, accepted structure, current/completed/archived task state, open/completed/skipped steps, candidate-only subagent findings, Commander admission status, script/function/call impact edges, dynamic PowerShell call risk, workspace read/write dataflow, mutation risk, recommended verification, and verification evidence instead of relying on vague chat memory.
- [VERIFIED] Evidence freshness gating is available through `scripts\evidence-freshness.ps1`; current-state/log/version conclusions must verify file name, timestamp, package version, and age before treating snapshots as live truth.
- [VERIFIED] Long-context safe learning is available through `scripts\learn-memory.ps1 -TextFile`, allowing compact draft files to be written and retried without relying on raw chat tails or oversized approval contexts.
- [VERIFIED] Multi-agent memory isolation requires helper/subagent findings to remain advisory/private until Commander/G1 review admits compact verified summaries through governed scripts; direct durable shared-memory writes are not allowed by default.
- [VERIFIED] Optional external extension packs live under `extensions/` and remain opt-in only: `karpathy-guidelines` for coding discipline/AI-style guidance and curated `mattpocock-skills` for TS/React/TDD/product workflows; core modules stay lightweight.
- [VERIFIED] Automatic maintenance policy is active: `maintenance-policy.json` defines bounded safe local hygiene, confirmation-required actions, never-automatic actions, evidence/archive requirements, and compaction continuation priority.
- [VERIFIED] Workspace lifecycle maintenance is active: `scripts/workspace-lifecycle-manager.ps1` plans or safely applies expired session-binding cleanup, expired Agent Bridge channel archival, stale active pointer cleanup, stale lock removal, generated temp-file cleanup, and evidence-draft visibility.
- [VERIFIED] Auto memory hygiene is active: `scripts/auto-hygiene-runner.ps1` detects overlong memories, duplicates, and private-pattern hits; low-risk overlong/duplicate cleanup archives original evidence before compact replacement/removal, while private hits require confirmation.
- [VERIFIED] Post-task maintenance hook is active: `scripts/post-task-maintenance.ps1` runs lifecycle cleanup, memory hygiene, self-improvement queue updates, state refresh, and status snapshots after task verification so safe maintenance does not wait for the user to ask.
- [VERIFIED] Self-improvement queue is active: `scripts/self-improvement-queue.ps1` turns hygiene/lifecycle/doctor/reflection/user-feedback gaps into candidate-only improvement items with no automatic skill mutation, no external publishing, and evidence-first promotion gates.
- [VERIFIED] Compression continuation priority is active: after context compaction, visible conversation, compressed records, active checkpoints, status snapshots/cards, step ledger, and recent tool results are current truth; long-term memory is supplemental and cannot override newer context.
...
- [VERIFIED] auto-check.ps1 uses the lightweight state cache first only when `lastVerifyOk=true`; stale or failed verification returns compact `verifySuggested=true` by default and runs full `verify-package.ps1` only with `-Force` or `-VerifyIfStale`, preventing accidental cold-start output bursts.
...
- [VERIFIED] super-brain-dashboard.ps1 provides the unified state control dashboard with Light/Full/Team modes: ordinary callers use Light state/status snapshots, Full adds roadmap/memory/privacy checks, and Team is required before team-task review gate checks wake up.
...
- [VERIFIED] smart-next.ps1 combines intent routing, auto-continuation, and Light dashboard state by default; dispatch learning and team review checks stay asleep unless intent-router classifies an explicit team/review/subagent request.
