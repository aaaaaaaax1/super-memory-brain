...
def _mcp_replay(root: Path, memory_root: str, require_recall_result: bool = True) -> dict[str, Any]:
    ...
    metadata_thread_id = "019fbc52-79e6-7941-af97-c1c2d40be451"
    desktop_metadata = {
        "threadId": metadata_thread_id,
        "x-codex-turn-metadata": {
            "thread_id": metadata_thread_id,
            "session_id": "mcp-replay-session-not-persisted",
            ...
            if content and isinstance(content[0], dict) and isinstance(content[0].get("text"), str):
                try:
                    ...
                    if tool_name == "brain_recall":
                        ...
                        nested_ok = nested_ok and len(nested) <= 4
                        nested_ok = nested_ok and sum(int(item.get("tokenEstimate", 0)) for item in nested) <= 500
    ...
    def nested_payload(index: int, expected_id: int, name: str) -> dict[str, Any] | None:
        envelope = take(index, expected_id)
        if not envelope or not isinstance(envelope.get("result"), dict):
            errors.append(f"tool_payload_contract:{name}")
            return None
        content = envelope["result"].get("content", [])
        ...
        try:
            ...
            errors.append(f"nested_json:{name}:{exc}")
            return None
...
def main() -> int:
    ...
    if args.mcp_only:
        ...
        memory_count = int(core.status().get("memoryCount", 0) or 0)
        replay = _mcp_replay(
            root,
            str(core.memory_root),
            require_recall_result=memory_count > 0,
        )
        checks = replay.get("checks", [])
        report = {
            "ok": bool(replay.get("ok")),
            "suite": "super-brain-mcp-smoke",
            "total": len(checks),
            "passed": sum(1 for item in checks if item.get("ok")),
            "failed": sum(1 for item in checks if not item.get("ok")),
            "memoryEmpty": memory_count == 0,
            "mcpReplay": replay,
        }
    ...
    for case in ([] if args.contract_only else cases):
        ...
        items = core.recall(
            ...
            str(case.get("queryDate", "")),
        )
        latency_ms = (time.perf_counter() - start) * 1000.0
        latencies.append(latency_ms)
        # Eval cases assert the public compact JSON contract exposed by the
        # CLI/MCP.  Default json.dumps spacing made an otherwise-correct
        # `sourceType` field fail a text-contract assertion.
        serialized = json.dumps(items, ensure_ascii=False, separators=(",", ":"))
        first = json.dumps(items[0], ensure_ascii=False, separators=(",", ":")) if items else ""
        ...
        output_budget_ok = len(items) <= 4 and sum(int(item.get("tokenEstimate", 0)) for item in items) <= 500
        ok = (
        ...
        results.append(
            {
                ...
                "outputBudgetOk": output_budget_ok,
