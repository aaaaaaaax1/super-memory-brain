...
## Advisory Experience Transfer

`topic_key`: `advisory-experience-transfer`
`decision_key`: `experience-is-reference-not-authority`

- For a continuity, planning, debugging, review, or other non-trivial task,
  preflight may first look for a compact relevant decision, verified procedure,
  prior failure pattern, or experience entry. This is a bounded method search,
  not an ordinary-chat recall.
- A retrieved experience is a low-priority advisory hypothesis. It can suggest
  a method, a known risk, or an option to compare; it cannot override the
  latest user instruction, live evidence, current task state, permissions,
  policy, safety, or an explicit scope boundary.
- Reuse needs independent current evidence. Without it the item remains
  `insufficient`, is not a fact or hard rule, and cannot trigger an automatic
  mutation, conclusion, or promotion.
- When no matching personal experience exists, use current evidence and
  first-principles reasoning, then compare any relevant reusable procedures,
  skills, or verified general methods as parallel options. Absence of memory
  must not narrow the model's search or force an old recipe onto a new task.
- Reflection may stage a compact, verified new lesson after meaningful
  outcomes. It improves future method selection only through the governed
  learning lifecycle; repeated reuse alone never proves that a method is good.
- Personal collaboration style is learned only through scoped user-adaptation
  evidence. It is not a freeform persona and never changes factual, safety,
  privacy, authorization, or permission rules.

## Governed Pitfall Experience

`topic_key`: `governed-pitfall-experience`
`decision_key`: `pitfall-is-evidence-not-a-universal-rule`

- A pitfall record is a compact, scoped negative experience, not a generic
  prohibition. It requires a verified trigger, root cause, prevention gate,
  evidence reference, and scope before it can be active.
- Preflight may surface a matching active pitfall only as an advisory card. It
  must check current independent evidence before reuse and cannot infer that a
  control, condition, or failure is absent in the current task.
- Each pitfall has a bounded revalidation deadline. A stale or due record may
  warn that prior evidence exists, but cannot drive a fix, conclusion,
  promotion, or automatic mutation until it is reconfirmed.
- A single failure never becomes a universal hard rule. Shared use still
  requires the existing confirmed-scope and verified-use gate.
