...
if ($TeamTaskId) {
  ...
  $record = Read-TeamTaskRecordLocked $path
  ...
  if (-not (Test-Path -LiteralPath $indexPath)) {
    $indexParameters = @{ Json = $true }
    if (-not [string]::IsNullOrWhiteSpace($StateRoot)) { $indexParameters.StateRoot = $StateRoot }
    & (Join-Path $PSScriptRoot 'team-task-index.ps1') @indexParameters | Out-Null
  }
  $index = Invoke-SuperBrainFileLock $indexPath {
    if (-not (Test-Path -LiteralPath $indexPath -PathType Leaf)) { throw 'TEAM_TASK_INDEX_MISSING' }
