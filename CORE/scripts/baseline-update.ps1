...
- [VERIFIED] direct Git privacy preflight excludes private memory, archives, machine paths, and generated output.

## Known Limitations

- [KNOWN_LIMITATION] plusunm-g1 currently works primarily as a skill-policy/governance layer; do not assume `python -m brain_memory` exists. Use package root markers and the PowerShell/NexSandglass script entry points under `scripts/` and the package-owned vendor runtime source.
- [PRIVACY] Keep memory/ local and Git-ignored; direct Git readiness must prove it is not staged.
