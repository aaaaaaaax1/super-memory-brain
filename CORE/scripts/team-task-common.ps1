...
function Read-TeamTaskRecordLocked([string]$Path) {
  return Invoke-TeamTaskRecordLock $Path {
    Read-TeamTaskRecord $Path
  }
}

function Get-TeamTaskRecordFilesStable(
  [string]$TeamRoot,
  [int]$TimeoutMs = 15000,
  [int]$RetryMilliseconds = 40
) {
  if (-not (Test-Path -LiteralPath $TeamRoot -PathType Container)) { return @() }

  $creationLock = Get-SuperBrainLockPath (Join-Path $TeamRoot '.team-task-create')
  $deadline = [DateTime]::UtcNow.AddMilliseconds($TimeoutMs)
  while ([DateTime]::UtcNow -lt $deadline) {
    $beforeLocks = @(Get-ChildItem -LiteralPath $TeamRoot -Filter 'team-*.json.lock' -File -ErrorAction SilentlyContinue)
    if ($beforeLocks.Count -gt 0 -or (Test-Path -LiteralPath $creationLock)) {
      Start-Sleep -Milliseconds $RetryMilliseconds
      continue
    }

    $files = @(Get-ChildItem -LiteralPath $TeamRoot -Filter 'team-*.json' -File -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending)
    $afterLocks = @(Get-ChildItem -LiteralPath $TeamRoot -Filter 'team-*.json.lock' -File -ErrorAction SilentlyContinue)
    if ($afterLocks.Count -eq 0 -and -not (Test-Path -LiteralPath $creationLock)) { return @($files) }
    Start-Sleep -Milliseconds $RetryMilliseconds
  }

  throw "TEAM_TASK_SCAN_LOCK_TIMEOUT root=$TeamRoot timeoutMs=$TimeoutMs"
}
