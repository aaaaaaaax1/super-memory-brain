...
if ([string]::IsNullOrWhiteSpace($DestinationRoot)) {
  $DestinationRoot = Join-Path (Get-SuperBrainWritableArchiveRoot $root) 'backups'
}
...
foreach ($platform in @('zcode','codex')) {
  ...
  foreach ($name in @(Get-SuperBrainSkillNames)) {
...
if ($IncludeWorkspace) {
  ...
  $criticalWorkspaceItems = @(
    ...
    'session-binding.json','session-notes.md','experience-index.md','guard-state','runtime-state','task-state-store','team-tasks','task-archive',
