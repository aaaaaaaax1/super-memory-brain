...
$currentTree = if($RequireEvidenceBinding){Get-SuperBrainSourceTreeBinding $Root}else{$null}
...
$items = @($candidates | ForEach-Object {
  ...
  $freshTime = $false
  $freshVersion = $false
  $freshBinding = (-not $RequireEvidenceBinding)
  $bindingStatus = if($RequireEvidenceBinding){'missing'}else{'not_requested'}
  $reason = @()
  if ($exists) {
    ...
    try {
      ...
      if ($RequireEvidenceBinding) {
        $binding = if($json.PSObject.Properties['evidenceBinding']){$json.evidenceBinding}else{$null}
        if (-not $binding) { $bindingStatus = 'historical_evidence_binding_missing'; $reason += $bindingStatus }
        elseif ([string]$binding.packageVersion -ne $RequiredVersion) { $bindingStatus = 'evidence_package_version_mismatch'; $reason += $bindingStatus }
        elseif ([string]$binding.treeAlgorithm -ne [string]$currentTree.treeAlgorithm -or [string]$binding.gitTreeHash -ne [string]$currentTree.gitTreeHash -or [string]$binding.gitHeadTreeHash -ne [string]$currentTree.gitHeadTreeHash) { $bindingStatus = 'evidence_git_tree_mismatch'; $reason += $bindingStatus }
        else { $freshBinding = $true; $bindingStatus = 'current' }
      }
    } catch {}
  ...
  [pscustomobject]@{
    ...
    version = $fileVersion
    evidenceBindingStatus = $bindingStatus
    fresh = ($exists -and $freshTime -and $freshVersion -and $freshBinding)
    reason = @($reason)
  }
})
...
$result = [pscustomobject]@{
  ...
  requiredVersion = $RequiredVersion
  maxAgeMinutes = $MaxAgeMinutes
  requireEvidenceBinding = [bool]$RequireEvidenceBinding
  items = @($items)
