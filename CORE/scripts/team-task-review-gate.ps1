...
function Get-TeamTaskFiles([string]$Id) {
  if (-not [string]::IsNullOrWhiteSpace($Id)) {
    ...
    return @([pscustomobject]@{ FullName=$path; Name=([IO.Path]::GetFileName($path)) })
  }
  return @(Get-TeamTaskRecordFilesStable $teamRoot)
}
...
foreach ($file in Get-TeamTaskFiles $TeamTaskId) {
  try { $record = Read-TeamTaskRecordLocked $file.FullName } catch {
    if (-not [string]::IsNullOrWhiteSpace($TeamTaskId)) { throw }
    $taskId = [IO.Path]::GetFileNameWithoutExtension($file.FullName)
    $blockers += [pscustomobject]@{ teamTaskId=$taskId; blocker='record_unreadable'; path=$file.FullName; reason=$_.Exception.Message }
    $taskSummaries += [pscustomobject]@{
      teamTaskId = $taskId
      userGoal = ''
      dispatchLevel = ''
      codeCapableDelegationCount = 0
      expectedJoinSlotCount = 0
      terminalJoinSlotCount = 0
      integratedJoinSlotCount = 0
      commanderDecisionStatus = 'unreadable'
