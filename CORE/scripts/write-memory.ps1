...
$stateWorkspace = Join-Path $stateRoot 'workspace'
if ([string]::IsNullOrWhiteSpace($WorkspaceKey)) { $WorkspaceKey = Get-SuperBrainWorkspaceKey '' }
$taskContext = Get-SuperBrainCurrentTaskContext $stateWorkspace $WorkspaceKey
if ([string]::IsNullOrWhiteSpace($TaskId) -and $taskContext -and $taskContext.PSObject.Properties['taskId']) { $TaskId = [string]$taskContext.taskId }
if ([string]::IsNullOrWhiteSpace($SessionId) -and $taskContext -and $taskContext.PSObject.Properties['sessionId']) { $SessionId = [string]$taskContext.sessionId }
if ([string]::IsNullOrWhiteSpace($SessionId) -and $env:SUPER_BRAIN_SESSION_ID) { $SessionId = [string]$env:SUPER_BRAIN_SESSION_ID }
$provenance = [ordered]@{}
if (-not [string]::IsNullOrWhiteSpace($SessionId)) { $provenance.sessionKey = Get-SuperBrainStableHash $SessionId 16 }
if (-not [string]::IsNullOrWhiteSpace($TaskId)) { $provenance.taskKey = Get-SuperBrainStableHash $TaskId 16 }
if (-not [string]::IsNullOrWhiteSpace($WorkspaceKey)) { $provenance.workspaceKey = Get-SuperBrainStableHash $WorkspaceKey 16 }
...
$provenance64 = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes(($provenance | ConvertTo-Json -Compress)))
python -c "import base64,json; from sandglass_log import log_message; text=base64.b64decode('$b64').decode('utf-8'); sender=base64.b64decode('$sender64').decode('utf-8'); provenance=json.loads(base64.b64decode('$provenance64').decode('utf-8')); print(log_message(text, sender, provenance))"
Write-Host "WRITE_OK score=$score private=$($privateHits.Count -gt 0) negative=$negativeHit layer=$Layer summary=$Summary provenance=$($provenance.Count -gt 0) memory=$MemoryRoot budgetStatus=$($memoryBudget.status) projectedLines=$($memoryBudget.projectedLines)/$($memoryBudget.maxLines)"
