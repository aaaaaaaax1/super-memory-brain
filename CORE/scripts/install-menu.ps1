...
function Install-Codex {
  Write-Host 'Installing the one Super Brain host entry for Codex with one shared memory root.'
  Write-Host 'ZCode remains an explicit compatibility target only; it is not part of the default install path.'
  Write-Host 'Bundled capability sources are absorbed by Super Brain and are not installed as independent host skills.'
...
function Show-Menu {
  ...
  Write-Host '1. Global inject / refresh available hosts + local runtime/MCP (shared memory; absorbed capabilities)'
