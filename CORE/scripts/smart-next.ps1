...
$dashboardMode = if ($intent.intent -eq 'team_or_review') { 'Team' } elseif ($intent.intent -in @('status','repository_readiness')) { 'Full' } else { 'Light' }
...
if ($intent.intent -eq 'workflow_preference_recall') {
  ...
  $confidence = 0.9
} elseif ($intent.intent -eq 'repository_readiness') {
  $nextAction = 'Run the direct Git repository and privacy preflight; keep private state ignored.'
  $commands = @('scripts\release-readiness.ps1 -Json')
  $why += 'repository_readiness_intent'
  $confidence = 0.88
...
if ($engineeringJudgmentRequired -and $intent.intent -ne 'add_or_optimize_feature') {
  ...
  if ($intent.intent -notin @('repository_readiness','status','team_or_review','agent_bridge_channel','workflow_preference_recall')) { $nextAction = 'Build an evidence-bounded engineering decision, resolve critical unknowns with the cheapest discriminating test, then execute and verify in dependency order.' }
}
