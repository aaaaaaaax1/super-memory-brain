...
if (-not $NoGraph) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($Supersedes)) {
    ...
    # The inverse link can point to a legacy non-ADR decision, so it must not create an ADR schema root.
...
$typedBindingRequested = ($Enforcement -eq 'completion_gate' -or @($StageKinds).Count -gt 0)
if ($typedBindingRequested) {
  if (@($StageKinds).Count -eq 0) { throw 'DECISION_BINDING_STAGE_KINDS_REQUIRED' }
  if ($Enforcement -eq 'completion_gate' -and -not $UserConfirmed) { throw 'DECISION_BINDING_USER_CONFIRMATION_REQUIRED' }
  $criteriaMaterial = if ([string]::IsNullOrWhiteSpace($CompletionCriteria)) { $Decision } else { $CompletionCriteria }
  if ($Enforcement -eq 'completion_gate' -and [string]::IsNullOrWhiteSpace($criteriaMaterial)) { throw 'DECISION_BINDING_COMPLETION_CRITERIA_REQUIRED' }
  $bindingScript = Join-Path $PSScriptRoot 'decision-binding.ps1'
  if (-not (Test-Path -LiteralPath $bindingScript -PathType Leaf)) { throw 'DECISION_BINDING_SCRIPT_MISSING' }
  $bindingArgs = @{
    Action='Register'; DecisionId=$Key; WorkspaceKey=(Get-SuperBrainWorkspaceKey $WorkspaceKey); TaskId=$TaskId; TaskInstanceId=$TaskInstanceId; WorklineId=$WorklineId; StageKinds=@($StageKinds); IntentFingerprint=$IntentFingerprint; Enforcement=$Enforcement; Lifecycle=if($Status -eq 'accepted'){'active'}else{'proposed'}; Authority=if($UserConfirmed){'user_confirmed'}else{'legacy'}; ContentHash=(Get-SuperBrainStableHash ($Question + "`n" + $Decision) 64); CompletionCriteriaDigest=(Get-SuperBrainStableHash $criteriaMaterial 64); PrivateGuidance=("Decision: " + $Decision + "`nAcceptance: " + $criteriaMaterial); Supersedes=$Supersedes; StateRoot=(Get-SuperBrainMemoryBaseRoot $Root); NoExit=$true; Json=$true
  }
  $bindingRaw = @(& $bindingScript @bindingArgs 2>&1)
  $decisionBinding = ConvertFrom-SuperBrainJsonOutput ($bindingRaw -join "`n") 'write decision execution binding'
  if (-not $decisionBinding -or $decisionBinding.ok -ne $true) { throw ('DECISION_BINDING_REGISTER_FAILED code=' + $(if($decisionBinding){[string]$decisionBinding.code}else{'unknown'})) }
}

Write-Host "DECISION_WRITE_OK key=$Key subject=$subject adr=$hasAdrFields status=$Status graph=$(-not $NoGraph) executionBinding=$([bool]$decisionBinding)"
