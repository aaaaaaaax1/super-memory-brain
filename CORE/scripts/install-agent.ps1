...
$requestedMode = $Mode
if ($Mode -ne 'Shared') { Write-Host "MEMORY_MODE_RETIRED requested=$Mode resolved=Shared reason=single-super-brain-memory-authority" }
$memoryRoot = Get-SuperBrainSharedMemoryRoot $Root
$scope = 'shared'
$members = @('all-agents')
Write-SuperBrainSharingPolicy $Root 'shared' $memoryRoot $members | Out-Null
...
Write-Host "AGENT_INSTALL_OK agent=$agent mode=Shared requestedMode=$requestedMode skillRoot=$SkillRoot memory=$memoryRoot absorbedCapabilities=package-owned-cold-sources policy=$(Get-SuperBrainSharingPolicyPath $Root)"
