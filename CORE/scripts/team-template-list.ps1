...
function New-DefaultAgentTeams {
  return [pscustomobject]@{
    ...
    templates = @(
      ...
      [pscustomobject]@{ name='Release Team'; id='release-team'; dispatchLevels=@('single_delegate','team_parallel','review_board'); triggers=@('release','share','hot_refresh','verification_required','repository_readiness','privacy_preflight'); roles=@('verify-runner','docs-checker','share-safety-checker'); purpose='Check verification, docs, direct-Git privacy, and hot-refresh readiness.' },
