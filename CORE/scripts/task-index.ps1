...
$statusRoot = Join-Path $workspace 'runtime-state\task-index'
$statusPath = Get-SuperBrainCanonicalTaskPath $statusRoot $workspaceKeyValue '.json'
foreach($dir in @($workspace,$statusRoot)){if(-not(Test-Path -LiteralPath $dir)){New-Item -ItemType Directory -Force -Path $dir|Out-Null}}
...
function Get-DefaultAgentId([string]$Platform, [string]$AgentName) {
  if ([string]::Equals($Platform,'super-brain',[StringComparison]::OrdinalIgnoreCase) -and [string]::Equals($AgentName,'super-memory-brain',[StringComparison]::OrdinalIgnoreCase)) { return 'super-brain-control-plane' }
...
function Get-SessionCardKey([string]$CandidateWorkspaceKey,[string]$CandidateSessionId) {
  if ([string]::IsNullOrWhiteSpace($CandidateWorkspaceKey) -or [string]::IsNullOrWhiteSpace($CandidateSessionId)) { return '' }
  return ((Get-SuperBrainWorkspaceKey $CandidateWorkspaceKey) + '|' + $CandidateSessionId.Trim()).ToLowerInvariant()
}
...
function New-TaskCandidate(
  ...
  $identityKey = ((Limit-Text $CandidateWorkspaceKey 120) + '|' + $agentIdValue + '|' + $sessionIdValue + '|' + $taskId).ToLowerInvariant()
  return [pscustomobject]@{
    ...
    workspaceKey = (Limit-Text $CandidateWorkspaceKey 120)
...
function Get-SessionCards {
  ...
  foreach ($file in @(Get-ChildItem -LiteralPath $dir -Filter '*.session.json' -File -ErrorAction SilentlyContinue)) {
    ...
    if (-not $card -or [string]::IsNullOrWhiteSpace([string]$card.sessionId) -or [string]::IsNullOrWhiteSpace([string]$card.workspaceKey)) { continue }
    if (-not (Test-SuperBrainWorkspaceKey ([string]$card.workspaceKey) $workspaceKeyValue)) { continue }
    $key = Get-SessionCardKey ([string]$card.workspaceKey) ([string]$card.sessionId)
    if (-not [string]::IsNullOrWhiteSpace($key)) { $sessions[$key] = $card }
  }
  return $sessions
}
...
function Add-SharedTaskCards([System.Collections.Hashtable]$SessionCards) {
  ...
  foreach ($state in @('active','paused','blocked','completed')) {
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $dir -Filter '*.task.json' -File -ErrorAction SilentlyContinue)) {
      ...
      if (-not $task.PSObject.Properties['workspaceKey'] -or [string]::IsNullOrWhiteSpace([string]$task.workspaceKey) -or -not (Test-SuperBrainWorkspaceKey ([string]$task.workspaceKey) $workspaceKeyValue)) { continue }
      $sessionCard = $null
      $sessionKey = Get-SessionCardKey ([string]$task.workspaceKey) ([string]$task.sessionId)
      if (-not [string]::IsNullOrWhiteSpace($sessionKey) -and $SessionCards.ContainsKey($sessionKey)) { $sessionCard = $SessionCards[$sessionKey] }
...
function Get-ExecutionContractCandidates {
  ...
  foreach ($file in @(Get-ChildItem -LiteralPath $contractRoot -Filter '*.json' -File -ErrorAction SilentlyContinue)) {
    ...
    if (-not $contract -or [string]$contract.schema -ne 'super-brain.execution-contract.v1') { continue }
    if ([string]$contract.status -ne 'active' -or [string]::IsNullOrWhiteSpace([string]$contract.taskId)) { continue }
    if (-not (Test-SuperBrainWorkspaceKey ([string]$contract.workspaceKey) $workspaceKeyValue)) { continue }
    if (-not $contract.planReceipt -or [string]::IsNullOrWhiteSpace([string]$contract.planReceipt.planFingerprint)) { continue }
    $taskId = [string]$contract.taskId
    $existing = $latestByTask[$taskId]
    if (-not $existing) {
      $latestByTask[$taskId] = [pscustomobject]@{ contract=$contract; path=$file.FullName }
      continue
    }
    $existingTime = try { [datetime]::Parse([string]$existing.contract.updatedAt) } catch { [datetime]::MinValue }
    $candidateTime = try { [datetime]::Parse([string]$contract.updatedAt) } catch { [datetime]::MinValue }
    if ($candidateTime -gt $existingTime) { $latestByTask[$taskId] = [pscustomobject]@{ contract=$contract; path=$file.FullName } }
  }

  foreach ($entry in @($latestByTask.Values)) {
    $contract = $entry.contract
    $plan = if ($contract.workLineStatus -and $contract.workLineStatus.activePlan) { $contract.workLineStatus.activePlan } else { $null }
    $taskName = if ($plan -and $plan.focusLabel) { [string]$plan.focusLabel } elseif ($contract.focusLabel) { [string]$contract.focusLabel } else { [string]$contract.taskId }
    $goal = if ($contract.assistantCommitment) { [string]$contract.assistantCommitment } else { $taskName }
    $items += New-TaskCandidate ([string]$contract.taskId) $goal 'active' 'execution-contract' 1.0 0 'current H7 scoped execution contract overrides compatibility task cards' ([string]$contract.updatedAt) ([string]$contract.currentStep) ([string]$contract.nextAction) @($contract.completedSteps) @($contract.pendingSteps) @($contract.evidence) 'super-memory-brain' 'super-brain-control-plane' 'super-brain' ([string]$contract.ownerSessionKey) $taskName $taskName ([string]$entry.path) @() ([string]$contract.workspaceKey)
  }
  return $items
}
...
$contractCandidates = @(Get-ExecutionContractCandidates)
$authoritativeTaskIds = @{}
foreach ($candidate in $contractCandidates) { $authoritativeTaskIds[[string]$candidate.taskId] = $true }
if ($authoritativeTaskIds.Count -gt 0) {
  $candidates = @($candidates | Where-Object { -not $authoritativeTaskIds.ContainsKey([string]$_.taskId) })
}
$candidates += $contractCandidates

$activeCheckpoint = Read-WorkspaceJson 'active-checkpoint.json'
if ($activeCheckpoint -and $activeCheckpoint.PSObject.Properties['workspaceKey'] -and (Test-SuperBrainWorkspaceKey ([string]$activeCheckpoint.workspaceKey) $workspaceKeyValue)) {
...
$currentTaskContext = Get-SuperBrainCurrentTaskContext $workspace $workspaceKeyValue
$taskGraphWorkspaceMatch = ($legacyTaskGraph -and $legacyTaskGraph.PSObject.Properties['workspaceKey'] -and (Test-SuperBrainWorkspaceKey ([string]$legacyTaskGraph.workspaceKey) $workspaceKeyValue))
$taskGraph = if ($taskGraphWorkspaceMatch) { $legacyTaskGraph } else { $null }
$ledgerTaskId = if ($currentTaskContext -and [string]$currentTaskContext.status -eq 'active' -and $currentTaskContext.stale -ne $true -and -not [string]::IsNullOrWhiteSpace([string]$currentTaskContext.taskId)) { [string]$currentTaskContext.taskId } else { '' }
$stepLedgerSelection = if (-not [string]::IsNullOrWhiteSpace($ledgerTaskId)) { Get-SuperBrainRelevantStepLedger $workspace $ledgerTaskId $workspaceKeyValue -AllowLegacyRead } else { $null }
$stepLedger = if ($stepLedgerSelection) { $stepLedgerSelection.ledger } else { $null }
if ($IncludeDiagnostic -and $taskGraphWorkspaceMatch -and -not [string]::IsNullOrWhiteSpace([string]$taskGraph.taskId)) {
  $status = if ($taskGraph.status) { [string]$taskGraph.status } else { 'unknown' }
  $candidates += New-TaskCandidate ([string]$taskGraph.taskId) ([string]$taskGraph.goal) 'legacy_read_only' 'task-graph.json (migration diagnostic)' 0.12 98 ('legacy graph status=' + $status + '; cannot establish current task or resume target') ([string]$taskGraph.updatedAt) '' '' @() @() @('task-graph.json') 'legacy-migration' 'legacy-non-authorizing' 'legacy' '' '' ([string]$taskGraph.goal) (Join-Path $workspace 'task-graph.json') @() ([string]$taskGraph.workspaceKey)
}

$lastCompletedCheckpoint = Read-WorkspaceJson 'last-completed-checkpoint.json'
if ($lastCompletedCheckpoint -and $lastCompletedCheckpoint.PSObject.Properties['workspaceKey'] -and (Test-SuperBrainWorkspaceKey ([string]$lastCompletedCheckpoint.workspaceKey) $workspaceKeyValue)) {
...
$result = [pscustomobject]@{
  ...
  rankingPolicy = 'rank/confidence/freshness: active execution contract rank 0 > shared task identity index rank 5 > compatibility checkpoint rank 10 > paused/blocked rank 30 > weak verification rank 80 > status-card fallback rank 90; legacy task graphs are diagnostic-only and cannot establish a current task.'
