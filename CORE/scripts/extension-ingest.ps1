...
function Inspect-Path([string]$CandidatePath){
  ...
  return [pscustomobject]@{
    ...
    canDo=@('candidate skill/plugin can be inspected, absorbed, and converted into a Super Brain-routable capability')
    ...
    suggestedAction='Use -Action Adopt only after inspection looks correct; it retains a package source and rebuilds the absorbed capability map without installing a standalone host skill.'
  }
}

if($Action -eq 'List'){
  # Keep the ingest list on the same public catalog boundary as installers
  # and the UI.  Internal absorbed-capability sources remain available to
  # Super Brain's compiler/routing path, but must never leak into a user
  # visible extension/source menu.
  $catalog = @(Get-SuperBrainExtensionCatalog $Root)
  $manifests = @{}
  foreach($manifest in @(Get-SuperBrainExtensionManifests @() $Root)) {
    $manifests[[string]$manifest.id] = $manifest
  }
  $items = @()
  foreach($extension in $catalog){
    $manifest = $manifests[[string]$extension.id]
    $skills = @()
    foreach($skillName in @($extension.skills)){
      $skill = @($manifest.skills | Where-Object { [string]$_.name -eq [string]$skillName } | Select-Object -First 1)
      $skills += [pscustomobject]@{ name=[string]$skillName; path=if($skill){[string]$skill.path}else{''}; triggers=if($skill){@($skill.triggers)}else{@()}; installedState=@(Get-InstalledState ([string]$skillName)) }
    }
    $items += [pscustomobject]@{ id=[string]$extension.id; name=[string]$extension.name; defaultEnabled=[bool]$extension.defaultEnabled; skillCount=[int]$extension.skillCount; skills=@($skills); manifestPath=if($manifest){[string]$manifest.manifestPath}else{''}; setupRequired=[string]$extension.setupRequired }
  }
  $result=[pscustomobject]@{ ok=$true; checkedAt=(Get-Date).ToString('yyyy-MM-dd HH:mm:ss'); schema='super-brain.extension-ingest.v1'; action=$Action; version=(Get-SuperBrainManifest $Root).version; count=@($items).Count; extensions=@($items); guard='Package source list is visibility for Super Brain-absorbed capabilities, not a host-skill install menu.'; path=$outPath }
}
...
elseif($Action -eq 'RebuildMap'){
  ...
  $manifest = [pscustomobject]@{
    id=$inspection.extensionId; name=$inspection.name; type='absorbed-capability-source'; defaultEnabled=$false; sourceRepo='local'; sourceCommit='local-adopted'; license='unknown'; installNote='Local adopted capability source; Super Brain owns routing and execution. Review setup and privacy before sharing.'
  ...
  $mapRaw = @(& (Join-Path $PSScriptRoot 'extension-capability-map.ps1') -Json 2>$null)
  $map = (($mapRaw -join "`n") | ConvertFrom-Json)
  $result=[pscustomobject]@{ ok=$true; checkedAt=(Get-Date).ToString('yyyy-MM-dd HH:mm:ss'); schema='super-brain.extension-ingest.v1'; action=$Action; version=(Get-SuperBrainManifest $Root).version; adopted=[pscustomobject]@{ capabilitySourceId=$inspection.extensionId; targetRoot=$targetRoot; sourceName=$inspection.skillName; sourcePath=$skillTarget; executionOwner='super-memory-brain'; standaloneInstall=$false }; extensionCapabilityMap=$map; nextAction='Run verify-extensions, skill-capability-map, and verify-package before relying on this absorbed capability.'; path=$outPath }
}
