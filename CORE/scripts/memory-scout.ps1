...
try {
  ...
  $currentTasks = @($taskIndex.current)
  $contractTasks = @($currentTasks | Where-Object { [string]$_.source -eq 'execution-contract' })
  if ($contractTasks.Count -eq 1) {
    $currentTasks = @($contractTasks)
  } elseif ($contractTasks.Count -gt 1) {
    $currentTasks = @()
  }
  foreach($task in @($currentTasks | Select-Object -First $TopK)) { Add-Card $cards 'current_task' ([string]$task.taskName) ([string]$task.currentStep + ' next=' + [string]$task.nextAction) ([string]$task.sourcePath) 0.9 }
} catch {}

$currentTaskContext = Get-SuperBrainCurrentTaskContext $workspace
if($currentTaskContext){ Add-Card $cards 'workspace_state' 'current-task-context.json' (($currentTaskContext | ConvertTo-Json -Depth 4 -Compress)) ([string]$currentTaskContext.contextProjectionPath) 0.65 }
foreach($name in @('active-checkpoint.json','last-verify-package.json','last-guard-negative-e2e.json','last-guard-flow-e2e.json','last-completion-guard.json','last-hot-refresh.json','last-reflection-promotion.json')) {
