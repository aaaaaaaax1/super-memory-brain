...
$cases += New-Case 'continue_restore_light_packet' ($executeGate.parsed -and $executeGate.value.intent -eq 'execute' -and $restore.parsed -and $restore.value.recallTriggered -eq $false -and $restore.value.executionResolution -and $restore.value.executionResolution.claimAllowed -eq $true -and -not [string]::IsNullOrWhiteSpace([string]$restore.value.executionResolution.taskId) -and -not [string]::IsNullOrWhiteSpace([string]$restore.value.executionResolution.nextAction)) 'continue must be authorized and restore a scoped light packet without deep recall' ([pscustomobject]@{ gate=$executeGate.value; restore=$restore.value })
