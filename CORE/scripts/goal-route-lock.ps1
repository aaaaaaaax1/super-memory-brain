...
function Get-ScopedPath([string]$Value) { if ([string]::IsNullOrWhiteSpace($Value)) { return '' }; return (Get-SuperBrainCanonicalTaskPath $goalScopeRoot $Value '.json') }
function Get-LegacyScopedPath([string]$Value) { $safe=Safe-TaskId $Value; if ([string]::IsNullOrWhiteSpace($safe)) { return '' }; return (Join-Path $goalScopeRoot ($safe + '.json')) }
...
function Update-CompatibilityLock([object]$ChangedLock,[switch]$RemoveChanged) {
  ...
  if($RemoveChanged){
    ...
    $candidates=@(Get-ActiveScopedLocks $changedTaskId|Group-Object taskId|ForEach-Object{$_.Group|Select-Object -First 1})
...
function Read-Lock { 
  ...
  $legacy = Get-LegacyScopedPath $TaskId
  foreach($candidate in @($scoped,$legacy)){
