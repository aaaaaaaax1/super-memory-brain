...
function ConvertTo-StatusStateCard([object]$Card) {
  ...
  return [pscustomobject]@{
    ...
    stateFingerprint = Limit-Text ([string]$Card.stateFingerprint) 16
    ...
    canonicalPlanId = if($Card.PSObject.Properties['canonicalPlanId']){Limit-Text ([string]$Card.canonicalPlanId) 80}else{''}
    canonicalGeneration = if($Card.PSObject.Properties['canonicalGeneration']){[int]$Card.canonicalGeneration}else{0}
    canonicalFingerprint = if($Card.PSObject.Properties['canonicalFingerprint']){Limit-Text ([string]$Card.canonicalFingerprint) 16}else{''}
    canonicalChecklistCount = if($Card.PSObject.Properties['activeChecklistCount']){[int]$Card.activeChecklistCount}else{@($Card.activeChecklist).Count}
    canonicalChecklistPreview = @($Card.activeChecklist | Select-Object -First 6 | ForEach-Object { [pscustomobject]@{ ordinal=[int]$_.ordinal; status=Limit-Text ([string]$_.status) 16; label=Limit-Text ([string]$_.label) 100 } })
    ...
    activeWorkPackageChecklistPreview = @($Card.activeWorkPackageChecklist | Select-Object -First 4 | ForEach-Object { [pscustomobject]@{ ordinal=[int]$_.ordinal; status=Limit-Text ([string]$_.status) 16; label=Limit-Text ([string]$_.label) 100 } })
...
$rawContractStateCard = if ($executionResolution -and $executionResolution.PSObject.Properties['continuityStateCard']) { $executionResolution.continuityStateCard } else { $null }
$contractStateCard = ConvertTo-StatusStateCard $rawContractStateCard
...
if ($currentTaskContextUsable -and $currentTaskContext.expiresAt) {
  try { $currentTaskContextUsable = ([DateTimeOffset]::Parse([string]$currentTaskContext.expiresAt,[Globalization.CultureInfo]::InvariantCulture).ToUniversalTime() -gt (Get-SuperBrainUtcNow)) } catch { $currentTaskContextUsable = $false }
}
...
$activeTaskId = if ($contractWorkspaceMatch) { [string]$executionResolution.taskId } elseif ($activeCheckpointUsable) { [string]$activeCheckpoint.taskId } elseif ($currentTaskContextUsable) { [string]$currentTaskContext.taskId } else { '' }
$stepLedgerSelection = if (-not [string]::IsNullOrWhiteSpace($activeTaskId)) { Get-SuperBrainRelevantStepLedger $workspace $activeTaskId $workspaceKeyValue -AllowLegacyRead } else { $null }
$stepLedger = if ($stepLedgerSelection) { $stepLedgerSelection.ledger } else { $null }
$continuitySource = if ($contractWorkspaceMatch) { 'execution-contract.ps1' } elseif ($activeCheckpointUsable) { $checkpointSelection.source } elseif ($currentTaskContextUsable) { 'current-task-context.json' } else { 'none' }
...
$contractStateCardUsable = ($contractWorkspaceMatch -and $null -ne $rawContractStateCard)
$contractCanonicalProgress = if ($contractWorkspaceMatch -and $executionResolution.workLineStatus -and $executionResolution.workLineStatus.PSObject.Properties['canonicalMain'] -and $executionResolution.workLineStatus.canonicalMain) { $executionResolution.workLineStatus.canonicalMain } else { $null }
$contractCanonicalCountsUsable = ($contractCanonicalProgress -and $contractCanonicalProgress.PSObject.Properties['completedCount'] -and $contractCanonicalProgress.PSObject.Properties['pendingCount'])

$openStepCount = if ($contractCanonicalCountsUsable) { [int]$contractCanonicalProgress.pendingCount } elseif ($contractStateCardUsable) { @($rawContractStateCard.pendingSteps).Count } elseif ($matchingActiveCheckpoint) { @($activeCheckpoint.pendingSteps).Count } elseif ($matchingStepLedger) { @($stepLedger.openSteps).Count } else { 0 }
$completedStepCount = if ($contractCanonicalCountsUsable) { [int]$contractCanonicalProgress.completedCount } elseif ($contractStateCardUsable) { @($rawContractStateCard.completedSteps).Count } elseif ($matchingActiveCheckpoint) { @($activeCheckpoint.completedSteps).Count } elseif ($matchingStepLedger) { @($stepLedger.completedSteps).Count } else { 0 }
$blockedStepCount = if ($contractStateCardUsable) { @($rawContractStateCard.blockers).Count } elseif ($matchingActiveCheckpoint) { @($activeCheckpoint.blockers).Count } elseif ($matchingStepLedger) { @($stepLedger.blockedSteps).Count } else { 0 }
...
$continuitySummary = [pscustomobject]@{
  ...
  taskStatus = if ($contractWorkspaceMatch) { 'active' } elseif ($matchingActiveCheckpoint) { [string]$activeCheckpoint.status } elseif ($matchingCurrentTaskContext) { [string]$currentTaskContext.status } else { '' }
  taskName = if ($contractWorkspaceMatch -and $executionResolution.workLineStatus -and $executionResolution.workLineStatus.activePlan) { Limit-Text ([string]$executionResolution.workLineStatus.activePlan.focusLabel) 180 } elseif ($matchingActiveCheckpoint) { Limit-Text ([string]$activeCheckpoint.taskName) 180 } else { '' }
  goal = if ($matchingActiveCheckpoint) { Limit-Text ([string]$activeCheckpoint.goal) 220 } elseif ($matchingCurrentTaskContext) { Limit-Text ([string]$currentTaskContext.acceptedGoal) 220 } else { '' }
  currentPhase = if ($contractStateCardUsable) { Limit-Text ([string]$rawContractStateCard.phase) 160 } elseif ($matchingActiveCheckpoint) { Limit-Text ([string]$activeCheckpoint.currentPhase) 160 } else { '' }
  currentStep = if ($contractStateCardUsable) { Limit-Text ([string]$rawContractStateCard.currentStep) 220 } elseif ($matchingActiveCheckpoint) { Limit-Text ([string]$activeCheckpoint.currentStep) 220 } elseif ($matchingStepLedger -and @($stepLedger.openSteps).Count -gt 0) { Limit-Text ([string]$stepLedger.openSteps[0].step) 220 } else { '' }
  ...
  # The same compact card is emitted at snapshot top level. Keep only an
  # availability flag here so the nested summary does not duplicate it.
  continuityStateCardAvailable = [bool]$contractStateCard
  source = $continuitySource
  consistency = $continuityConsistency
  legacyCompatibility = [pscustomobject]@{ taskGraphObserved=[bool]$taskGraphWorkspaceMatch; taskGraphApplied=$false; guard='legacy global task graph is migration-only and cannot establish current task state' }
