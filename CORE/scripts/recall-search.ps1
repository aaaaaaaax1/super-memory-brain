...
$TopK = [Math]::Min(4, [Math]::Max(1, $TopK))
$MaxTokens = [Math]::Min(500, [Math]::Max(32, $MaxTokens))
$runtimeQueryDate = if ([string]::IsNullOrWhiteSpace($QueryDate)) { (Get-Date).ToString('yyyy-MM-dd') } else { $QueryDate }
...
$cardSnippetChars = [Math]::Max(80, ($cardSnippetTokens * 4))
  $maxEvidenceCards = if ($contextBudget -and $contextBudget.PSObject.Properties['maxEvidenceCards']) { [Math]::Min(4, [Math]::Max(1, [int]$contextBudget.maxEvidenceCards)) } else { 4 }
...
if (-not $runtimeDisabled -and (Test-Path -LiteralPath $runtimeCli)) {
  try {
    $runtimeOutput = @(& python $runtimeCli --package-root $Root --memory-root $MemoryRoot --base64 recall --query $Query --top-k $TopK --max-tokens $MaxTokens --layer $Layer --query-date $runtimeQueryDate 2>$null)
...
function Get-SessionBindingSnippets([object[]]$Terms) {
  # RETIRED: memory\workspace\session-binding.json is legacy read-only
  # migration evidence. It is never a sessionBinding / temporary_session_binding
  # recall source and can never project [CURRENT] or executable nextAction state.
  # TTL markers such as expiresAt, root checks such as Test-SuperBrainSamePath,
  # and the old evidenceCards remain diagnostic concerns in session-binding.ps1.
  # H7's workspace/session/task scoped execution contract is the only
  # continuity authority. Keep this compatibility function empty so older
  # callers cannot silently re-enable the global injection route.
  return @()
}
...
# The retired compatibility function intentionally returns no candidates.
$null = @(Get-SessionBindingSnippets $terms)

$sandglassVaultModule = Join-Path $MemoryScripts 'sandglass_vault.py'
$memoryPythonReady = Test-Path -LiteralPath $sandglassVaultModule -PathType Leaf

function Invoke-SuperBrainMemoryPython([string]$Code) {
  # Legacy/offline state roots used by compatibility tests and old installs do
  # not carry a Python Sandglass module tree.  They are read-only evidence,
  # never a reason to emit a Python traceback or revive a retired session
  # binding.  Skip that optional search path cleanly and retain normal local
  # state-only recall.
  if (-not $memoryPythonReady) { return '' }
  $oldNexSandBase = $env:NEXSANDBASE_HOME
  ...
  try {
    ...
    if ($null -eq $oldNexSandBase) { Remove-Item Env:\NEXSANDBASE_HOME -ErrorAction SilentlyContinue } else { $env:NEXSANDBASE_HOME = $oldNexSandBase }
...
try {
  $result = Invoke-SuperBrainMemoryPython $code
...
if ($memoryPythonReady -and $candidates.Count -lt $TopK -and [bool]$hybrid.fallbackRecentWhenBelowTopK) {
  $recentCode = "import json; from sandglass_vault import recent; print(json.dumps(recent($candidateLimit)))"
  try {
    $recentResult = Invoke-SuperBrainMemoryPython $recentCode
    if (-not [string]::IsNullOrWhiteSpace($recentResult)) {
