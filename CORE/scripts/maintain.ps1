...
$result = [pscustomobject]@{
  ...
  separatelyAuthorizedH7Actions = $separatelyAuthorizedH7Actions
  retiredHookRepair = [pscustomobject]@{
    state = 'withheld'
    code = 'SUPER_BRAIN_HOOK_REPAIR_RETIRED'
    hookWriteAllowed = $false
    nextAction = 'Use host-cache-check.ps1 to diagnose H7; authorize H7 MCP repair separately if needed.'
  }
  steps = $steps
}
