...
if ($Status -in @('completed','verified')) {
  $failure = [pscustomobject]@{
    ok = $false
    errorCode = 'TASK_REGISTER_TERMINAL_STATUS_REQUIRES_COMPLETION_TRANSACTION'
    status = $Status
    taskId = $TaskId
    guard = 'Terminal task state must be committed through TaskStateStore CompleteTask after task-scoped verification.'
  }
  if ($Json) { $failure | ConvertTo-Json -Depth 4 } else { Write-Host "TASK_REGISTER_FAILED errorCode=$($failure.errorCode) status=$Status taskId=$TaskId" }
  exit 1
}
...
if (-not [string]::IsNullOrWhiteSpace($SessionId)) {
  $sessionScopeId = Get-SuperBrainTaskWorkspaceToken $SessionId $workspaceKeyValue
...
# TaskStateStore only accepts the canonical tokenized path. Keep this fast-path
# projection aligned with checkpoint-writer so a later registration overwrites
# the same task card instead of creating a competing legacy alias.
$taskCardPath = Get-SuperBrainCanonicalTaskPath $taskDir $TaskId '.task.json'
...
foreach ($other in @('active','paused','blocked','completed')) {
  ...
  $otherPath = Get-SuperBrainCanonicalTaskPath (Join-Path $tasksRoot $other) $TaskId '.task.json'
