...
foreach ($file in @(Get-TeamTaskRecordFilesStable $teamRoot)) {
  try { $record = Read-TeamTaskRecordLocked $file.FullName } catch {
    $unreadableRecords += [pscustomobject]@{ path=$file.FullName; reason=$_.Exception.Message }
    continue
  }
...
$result = [pscustomobject]@{
  ok = ($unreviewed -eq 0 -and $driftRisk -eq 0 -and $missingAuth -eq 0 -and $unreadableRecords.Count -eq 0)
  ...
  unreadableRecordCount = $unreadableRecords.Count
  unreadableRecords = @($unreadableRecords | Select-Object -First 10)
...
if ($Json) {
  ...
  Write-Host "TEAM_TASK_AUDIT codeCapable=$codeCapable unreviewed=$unreviewed drift=$driftRisk missingAuth=$missingAuth unreadable=$($unreadableRecords.Count) blocked=$blockedScope"
