...
function Test-SameEvidenceBinding($Left,$Right,[switch]$IncludeArtifactHash) {
  if (-not $Left -or -not $Right) { return $false }
  $fields = @('schema','packageVersion','gitTreeHash','treeAlgorithm','gitHeadTreeHash','taskId','workspaceKey','ownerSessionKey','artifactKind')
  if ($IncludeArtifactHash) { $fields += 'artifactHash' }
  foreach ($field in $fields) {
    if ([string](Get-PropertyValue $Left $field '') -ne [string](Get-PropertyValue $Right $field '')) { return $false }
  }
  return $true
}
...
function Get-OutcomeAssessment($Record, [string]$Path) {
  ...
  $evidenceBinding = Get-PropertyValue $Record 'evidenceBinding' $null
  $verificationPath = if ([string]::IsNullOrWhiteSpace($taskId)) { '' } else { Get-SuperBrainCanonicalTaskPath (Join-Path $workspace 'runtime-state\task-verifications') $taskId '.json' }
  $projectionPath = if ([string]::IsNullOrWhiteSpace($taskId)) { '' } else { Get-SuperBrainCanonicalTaskPath (Join-Path $workspace 'task-state-store\projections') $taskId '.json' }
  if (-not $evidenceBinding) {
    [void]$reasons.Add('historical_evidence_binding_missing')
  } else {
    try {
      $bindingCheck = Test-SuperBrainEvidenceBinding -Binding $evidenceBinding -TaskId $taskId -WorkspaceKey $effectiveWorkspaceKey -OwnerSessionKey ([string](Get-PropertyValue $evidenceBinding 'ownerSessionKey' '')) -ArtifactPath $verificationPath -RequireArtifactHash -Root $Root
      if (-not $bindingCheck.ok) { [void]$reasons.Add(('evidence_binding_' + [string]$bindingCheck.reason)) }
    } catch {
      [void]$reasons.Add('evidence_binding_validation_failed')
    }
    $verificationArtifact = Read-JsonFile $verificationPath
    if (-not $verificationArtifact -or -not (Get-PropertyValue $verificationArtifact 'evidenceBinding' $null)) {
      [void]$reasons.Add('verification_artifact_binding_missing')
    } elseif (-not (Test-SameEvidenceBinding $evidenceBinding $verificationArtifact.evidenceBinding)) {
      [void]$reasons.Add('verification_artifact_binding_mismatch')
    }
    $projection = Read-JsonFile $projectionPath
    $lifecycle = Get-PropertyValue $projection 'lifecycle' $null
    if (-not $projection -or [string](Get-PropertyValue $projection 'taskId' '') -ne $taskId -or -not $lifecycle -or [string](Get-PropertyValue $lifecycle 'status' '') -ne 'completed') {
      [void]$reasons.Add('completion_evidence_projection_invalid')
    } elseif ([string](Get-PropertyValue $lifecycle 'ownerSessionKey' '') -ne [string](Get-PropertyValue $evidenceBinding 'ownerSessionKey' '') -or -not (Test-SameEvidenceBinding $evidenceBinding (Get-PropertyValue $lifecycle 'evidenceBinding' $null) -IncludeArtifactHash)) {
      [void]$reasons.Add('completion_evidence_projection_binding_mismatch')
    }
  }
  ...
  return [pscustomobject]@{
    ...
    historical = (@($reasons | Where-Object { [string]$_ -like 'historical_*' -or [string]$_ -like 'evidence_binding_evidence_*' }).Count -gt 0)
    reasons = @($reasons)
...
try {
  ...
  $result = [pscustomobject]@{
    ...
    provenance = [pscustomobject]@{
      ...
      guard = 'Only a current source-tree-bound, task-scoped verified outcome can count; records missing the evidence binding are historical. Autonomy additionally requires a matching governed authorization, and correction closure additionally requires an immutable link to that outcome.'
    }
    records = [pscustomobject]@{
      ...
      historicalOutcomeCount = @($assessments | Where-Object { $_.historical }).Count
