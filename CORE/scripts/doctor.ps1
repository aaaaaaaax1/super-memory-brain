...
$coreChecks = @($summary,$skillSync,$memoryHealth,$scriptTiers,$toolHealth)
foreach ($item in $coreChecks) {
...
if (-not $taskLifecycle.data) {
  Add-DoctorRisk 'medium' 'task_lifecycle_audit_unavailable' 'Task lifecycle maintenance audit did not return readable evidence; exact current-task recall remains scope-gated.' 'task-lifecycle-audit.ps1'
} elseif (-not $taskLifecycle.ok -or $taskLifecycle.data.reconciliationRequired -eq $true) {
  $hardFindings = [int]$taskLifecycle.data.counts.hardFindings
  $conflicts = [int]$taskLifecycle.data.counts.completionEvidenceConflicts
  $resolvedPaths = if ($taskLifecycle.data.counts.PSObject.Properties['migratedProjectionPathsResolved']) { [int]$taskLifecycle.data.counts.migratedProjectionPathsResolved } else { 0 }
  Add-DoctorRisk 'medium' 'task_lifecycle_reconciliation_required' "Historical task lifecycle evidence requires scoped reconciliation (hardFindings=$hardFindings conflicts=$conflicts migratedPathsResolved=$resolvedPaths); ambiguous tasks stay fail-closed without disabling core recall." 'task-lifecycle-audit.ps1'
}
if (-not $startupCoreAvailable) {
  Add-DoctorRisk 'high' 'check_failed' "startup-check.ps1 exited with $($startup.exitCode) and core startup is unavailable." 'startup-check.ps1'
...
$result = [pscustomobject]@{
  ...
  maintenanceOk = [bool]($taskLifecycle.ok -and $taskLifecycle.data -and $taskLifecycle.data.reconciliationRequired -ne $true)
  maintenanceState = if (-not $taskLifecycle.data) { 'audit_unavailable' } elseif ($taskLifecycle.ok -and $taskLifecycle.data.reconciliationRequired -ne $true) { 'available' } else { 'reconciliation_required' }
