...
function Read-TaskStateProjection([string]$Id) {
  if ([string]::IsNullOrWhiteSpace($Id)) { return $null }
  $projectionRoot = Join-Path $workspace 'task-state-store\projections'
  $path = Get-SuperBrainCanonicalTaskPath $projectionRoot $Id '.json'
  $value = Read-JsonFile $path
  if ($value -and [string]$value.taskId -eq $Id) { return $value }
  return $null
}
...
$planSha256 = if (-not [string]::IsNullOrWhiteSpace($PlanPath)) { Get-SuperBrainFileSha256 $PlanPath } else { '' }
$projection = Read-TaskStateProjection $TaskId
$lifecycle = if ($projection -and $projection.PSObject.Properties['lifecycle']) { $projection.lifecycle } else { $null }
$bindingReason = 'task_state_projection_missing_or_unbound'
$bindingCurrent = $false
if ($projection -and $lifecycle -and [string]$lifecycle.status -eq 'active' -and -not [string]::IsNullOrWhiteSpace([string]$lifecycle.workspaceKey) -and -not [string]::IsNullOrWhiteSpace([string]$lifecycle.ownerSessionKey) -and [int]$lifecycle.contractRevision -gt 0 -and -not [string]::IsNullOrWhiteSpace([string]$lifecycle.planFingerprint) -and $planSha256 -match '^[a-f0-9]{64}$') {
  $bindingCurrent = $true
  $bindingReason = 'bound_to_active_task_state'
}
$sourceTreeBinding = $null
if ($bindingCurrent -and $gaps.Count -eq 0) {
  try { $sourceTreeBinding = Get-SuperBrainSourceTreeBinding $Root }
  catch { $bindingCurrent = $false; $bindingReason = 'source_tree_binding_failed' }
}
$result = [pscustomobject]@{
  ok = ($gaps.Count -eq 0)
  checkedAt = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
  schema = 'super-brain.causal-change-review.v2'
  ...
  reviewBinding = [pscustomobject]@{
    schema = 'super-brain.causal-review-binding.v1'
    status = if($bindingCurrent){'bound'}else{'unbound'}
    reason = $bindingReason
    taskId = Limit-Text $TaskId 120
    packageVersion = $currentVersion
    taskStateRevision = if($projection){[int]$projection.revision}else{0}
    workspaceKey = if($lifecycle){[string]$lifecycle.workspaceKey}else{''}
    ownerSessionKey = if($lifecycle){[string]$lifecycle.ownerSessionKey}else{''}
    contractRevision = if($lifecycle){[int]$lifecycle.contractRevision}else{0}
    planFingerprint = if($lifecycle){[string]$lifecycle.planFingerprint}else{''}
    planPath = $PlanPath
    planSha256 = $planSha256
    sourceTreeBinding = $sourceTreeBinding
    reviewFingerprint = ''
    producer = 'causal-change-review.ps1'
  }
...
$result.reviewBinding.reviewFingerprint = Get-SuperBrainCausalReviewFingerprint $result
