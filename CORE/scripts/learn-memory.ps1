...
$similarOutput = @()
try {
  $similarOutput = @(& (Join-Path $PSScriptRoot 'recall-search.ps1') -Query $Title -TopK 5 -MaxTokens 700 -Layer $layerForWrite -MemoryMode auto -Json 2>&1 3>$null 4>$null 5>$null 6>$null)
...
$writeOutput = @(& (Join-Path $PSScriptRoot 'write-memory.ps1') @writeParams 2>&1 3>$null 4>$null 5>$null 6>$null)
$writeOk = ($LASTEXITCODE -eq 0)

$experience = $null
if ($writeOk -and $Layer -eq 'experience') {
  $id = ('learn-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
  $experienceOutput = @(& (Join-Path $PSScriptRoot 'write-experience.ps1') -Id $id -Title $Title -Triggers @($Tags) -Scope 'project' -Do @($clean) -Evidence @($Evidence) -RecallQuery $Title -Json 2>&1 3>$null 4>$null 5>$null 6>$null)
  $experience = ($experienceOutput -join "`n")
}

$profileCard = $null
if ($writeOk -and $Layer -eq 'profile') {
  $profileOutput = @(& (Join-Path $PSScriptRoot 'profile-card.ps1') -Refresh -PreferredQuery $Title -Json 2>&1 3>$null 4>$null 5>$null 6>$null)
