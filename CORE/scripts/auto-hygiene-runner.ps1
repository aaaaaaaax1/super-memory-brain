...
function Invoke-RebuildMemoryIndexes([hashtable]$LineMap) {
  return Invoke-SuperBrainMemoryIndexRebuild $memoryRoot $LineMap
}
...
$sourceHash = if (Test-Path -LiteralPath $memoryPath -PathType Leaf) { Get-SuperBrainFileSha256 $memoryPath } else { '' }
$rewriteTransaction = [pscustomobject]@{ ok=$true; changed=$false; skipped=$true; reason='no_memory_change' }
...
if (Test-Path -LiteralPath $memoryPath) {
  ...
  foreach ($line in $originalLines) {
    ...
    if ($record.expired) {
      if ($actions.Count -lt $MaxActions) { Add-Action 'expired' $lineNumber 'archive_expired_with_original_archive' 'Explicit expiry has passed; expired memory is not eligible for default recall.' $current '' $false 'low' }
    ...
    if ($seen.ContainsKey($key)) {
      $lineMap[[string]$lineNumber] = [int]$seen[$key]
      if ($actions.Count -lt $MaxActions) {
        Add-Action 'duplicate' $lineNumber 'remove_duplicate_with_archive' 'Exact duplicate after timestamp/source normalization.' $current '' $false 'low'
      }
      if ($ApplySafe) {
      ...
      $seen[$key] = $newLineNumber + 1
    }

    if ($current.Length -gt $MaxMemoryChars) {
      $compact = New-CompactLine $current
      if ($actions.Count -lt $MaxActions) { Add-Action 'too_long' $lineNumber 'compress_with_original_archive' ('chars ' + $current.Length + ' > ' + $MaxMemoryChars) $current $compact $false 'low' }
...
if ($ApplySafe -and $changed) {
  try {
    ...
    $rewriteTransaction = Invoke-SuperBrainMemoryRewriteTransaction -MemoryRoot $memoryRoot -MemoryPath $memoryPath -ReplacementText (ConvertTo-SuperBrainMemoryRewriteText @($newLines)) -ExpectedSourceHash $sourceHash -LineMap $lineMap -TransactionKind 'hygiene' -EvidencePath $archivePath -ExpectedEvidenceHash (Get-SuperBrainFileSha256 $archivePath)
    $indexRebuild = [pscustomobject]@{ ok=$true; skipped=$false; value=$rewriteTransaction.indexRebuild }
    $rewriteCommitted = [bool]$rewriteTransaction.changed
    if ($rewriteCommitted) {
      foreach ($action in @($actions | Where-Object { $_.type -in @('expired','duplicate','too_long') })) { $action.applied = $true }
    }
