...
try {
  ...
  foreach ($item in @($(if($ApplySafe){$compact.blocked}else{$compact.candidates | Where-Object {$_.blocked}}))) {
    ...
    if ([string]::IsNullOrWhiteSpace($reason)) { $reason = [string]$item.blockReason }
    $action = if ($reason -eq 'incomplete_transaction') { 'blocked_pending_transaction' } elseif ($reason -eq 'active_or_unresolved_lifecycle') { 'blocked_active_or_unresolved_task' } else { 'blocked_compaction_guard' }
    $detail = "reason=$reason; lifecycle=$($item.lifecycleStatus); pending=$($item.pendingTransactionCount); reconcile or close the task before compaction"
    Add-Action 'task_state_journal' ([string]$item.taskId) $action $detail $false '' 'medium'
  }
} catch {
...
try {
  ...
  foreach ($item in @($taskLifecycle.cardCheckpointDivergences | Where-Object { -not $_.diagnostic })) {
    Add-Action 'task_card_checkpoint_drift' ([string]$item.sourcePath) 'requires_confirmation' ("task card has no pending steps while its scoped checkpoint has $($item.checkpointPendingCount); refresh the card from the checkpoint before task completion; taskId=$($item.taskId)") $false '' 'low'
  }
} catch {
