...
[void]$steps.Add((Invoke-JsonStep 'user-adaptation' {
  $adaptationScript=Join-Path $PSScriptRoot 'user-adaptation.ps1'
  if(-not$ApplySafe){& $adaptationScript -Action Status -Json;return}
  $statusRaw=@(& $adaptationScript -Action Status -Json)
  $status=(($statusRaw-join"`n")|ConvertFrom-Json)
  & $adaptationScript -Action Synthesize -ExpectedRevision ([int]$status.revision) -TransitionId ("post-task-synthesis-r$([int]$status.revision)") -Json
}))
