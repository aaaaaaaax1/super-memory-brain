...
$scenarios = @(
  ...
  [pscustomobject]@{ name='repository_readiness'; prompt='check repository Git status and privacy readiness'; kind='dispatch'; flags='VerificationRequired,Parallelizable'; extraReasons=@('repository_readiness','privacy_preflight'); expectedLevel='single_delegate'; expectedTemplate='release-team' },
...
foreach ($scenario in $scenarios) {
  ...
  switch ($scenario.name) {
    ...
    'repository_readiness' { $dispatchJsonText = & (Join-Path $PSScriptRoot 'team-dispatch-check.ps1') -VerificationRequired -Parallelizable -Json }
