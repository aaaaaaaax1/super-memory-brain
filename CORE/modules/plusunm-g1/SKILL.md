...
Sharing policy: Super Brain owns one shared memory authority. When durable memory must not be used for a turn, use `memory:off`; do not create a ZCode, Codex, Agent, or group-specific replacement root.
...
The local runtime is the package-owned NexSandglass source under `<package-root>\vendor\NexSandglass-Agent-DedicatedMemory`; the active `<memory-root>` contains data and indexes only. If a Python import is needed, set `NEXSANDBASE_HOME=<memory-root>` and `PYTHONPATH=<package-root>\vendor\NexSandglass-Agent-DedicatedMemory`, then import the documented `sandglass_*` modules. If a script or import fails, report the unavailable runtime and fall back to explicit local markdown/json memory files.
