...
For a specialized task, use `Search` once and select at most one exact
capability match. A high-confidence capability match or exact user-selected
name is automatically loaded into the current task from its verified
`skillFile`; it is not merely reported as available. Cold storage affects
catalog visibility, not current-task callability. Do not move it active and do
not require a new conversation.

`disable-model-invocation: true` excludes a skill from broad semantic matching,
but an exact user request can still resolve it. A wrapper may declare one
`delegates-to: <verified-skill-name>` target; the router verifies both files and
loads the target for this task. Multi-hop delegation, missing targets, hash
mismatches, and ambiguous matches fail closed. Global hot-pool promotion remains
an explicit profile-management operation, never an automatic routing side effect.

When the runtime has a high-confidence capability match, it must bind that one
verified skill for the current task and follow its `SKILL.md`; this is execution
selection, not a request for the user to name, activate, move, or restart it.
...
This router is a package-owned internal maintenance subroute of Super Memory
Brain. `super-memory-brain` is the only host-facing entry; keep this module in
the package/cold pool and never install it as an independent host skill.
...
Run report mode from the package-owned cold module (resolve `package-root.txt`
from `super-memory-brain` first):

```powershell
& "<package-root>\modules\skill-pool-router\scripts\manage-skill-pool.ps1" -Action Report -Json
```

Apply the protected default profile:

```powershell
& "<package-root>\modules\skill-pool-router\scripts\manage-skill-pool.ps1" -Action Apply -Json
```

Activate one cold skill:

```powershell
& "<package-root>\modules\skill-pool-router\scripts\manage-skill-pool.ps1" -Action Activate -SkillName <name> -Json
```

Expose a cold skill to new Codex tasks without moving or copying its source:

```powershell
& "<package-root>\modules\skill-pool-router\scripts\manage-skill-pool.ps1" -Action Expose -SkillName <name> -Json
```

Remove only that active junction while preserving the cold source:

```powershell
& "<package-root>\modules\skill-pool-router\scripts\manage-skill-pool.ps1" -Action Hide -SkillName <name> -Json
```

Resolve a named active or cold skill for immediate use without moving it:

```powershell
& "<package-root>\modules\skill-pool-router\scripts\manage-skill-pool.ps1" -Action Resolve -SkillName <name> -Json
```

Search by capability when no active skill fits:

```powershell
& "<package-root>\modules\skill-pool-router\scripts\manage-skill-pool.ps1" -Action Search -Query <capability> -Json
```

Rebuild the cold index without changing the active pool:

```powershell
& "<package-root>\modules\skill-pool-router\scripts\manage-skill-pool.ps1" -Action Reindex -Json
```
...
```powershell
& "<package-root>\modules\skill-pool-router\scripts\manage-skill-pool.ps1" -Action Restore -ManifestPath <manifest> -Json
```
