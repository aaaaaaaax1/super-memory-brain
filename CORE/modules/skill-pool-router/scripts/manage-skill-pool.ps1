...
$protectedFolders = @('.system','super-memory-brain','share-mini-imagegen',$freeImageFolder)
...
function Read-Metadata([string]$Folder) {
  ...
  if (-not $file) { return [pscustomobject]@{ name=(Split-Path -Leaf $Folder); description=''; skillFile=''; sha256=''; disableModelInvocation=$false; delegatesTo='' } }
  ...
  for($lineIndex=0;$lineIndex -lt $lines.Count;$lineIndex++) {
    ...
    if ($line -match '^disable-model-invocation:\s*(.*)$') {
      $disableModelInvocation = (([string]$Matches[1]).Trim().Trim('"').Trim("'").ToLowerInvariant() -in @('true','yes','1'))
    }
    if ($line -match '^delegates-to:\s*(.*)$') { $delegatesTo = ([string]$Matches[1]).Trim().Trim('"').Trim("'") }
...
function Get-ColdEntries {
  ...
  foreach($dir in @(Get-SkillCatalogDirectories $cold | Where-Object{-not $_.Name.StartsWith('.') -and $_.Name -ne 'manifests'})){
    ...
    $entries += [pscustomobject]@{folder=$dir.Name;name=$meta.name;description=$meta.description;coldPath=$dir.FullName;activePath=(Join-Path $active $dir.Name);skillFile=$meta.skillFile;sha256=$meta.sha256;disableModelInvocation=$meta.disableModelInvocation;delegatesTo=$meta.delegatesTo}
  }
...
function Get-ActiveEntries {
  ...
  foreach($file in @(Get-SkillCatalogFiles $active)){
    ...
    if([string]::IsNullOrWhiteSpace($meta.skillFile)){continue}
    $entries += [pscustomobject]@{source='active';folder=$folder;name=$meta.name;description=$meta.description;activePath=$folderPath;coldPath='';skillFile=$meta.skillFile;sha256=$meta.sha256;disableModelInvocation=$meta.disableModelInvocation;delegatesTo=$meta.delegatesTo}
  }
  return @($entries|Sort-Object folder)
}
