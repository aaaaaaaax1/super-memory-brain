...
Describe 'Skill pool content health' {
  ...
  It 'preserves invocation and one-hop delegate metadata in the cold index' {
    $skill = Join-Path $cold 'wrapper\SKILL.md'
    New-Item -ItemType Directory -Force -Path (Split-Path -Parent $skill) | Out-Null
    [IO.File]::WriteAllText($skill,"---`nname: wrapper`ndescription: Wrapper fixture.`ndisable-model-invocation: true`ndelegates-to: target-skill`n---`n",$utf8)

    $output = & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $scriptPath -Action Reindex -ActiveRoot $active -ColdRoot $cold -Json
    $LASTEXITCODE | Should Be 0
    $result = ($output -join "`n") | ConvertFrom-Json
    $result.ok | Should Be $true
    $index = Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $cold 'skill-pool-index.json') | ConvertFrom-Json
    $entry = @($index.entries | Where-Object { $_.name -eq 'wrapper' }) | Select-Object -First 1
    $entry.disableModelInvocation | Should Be $true
    $entry.delegatesTo | Should Be 'target-skill'
  }
}
