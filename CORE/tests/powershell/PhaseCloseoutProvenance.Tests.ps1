...
function Invoke-PhaseCloseoutContract([string]$HostRoot,[string[]]$Arguments) {
...
function Invoke-PhaseCloseoutH7([string]$StateRoot,[string]$HostRoot,[string]$SessionKey,[string]$Phase='evidence',[string]$ProgressCheckpointBase64='') {
...
function New-PhaseCloseoutProof([string]$HostRoot,[string]$Phase,[string]$Step,[string]$NextAction) {
  $evidencePath = Join-Path $HostRoot 'phase-evidence.txt'
...
function New-PhaseCloseoutFixture([switch]$ActivateH7,[string]$Phase='P7',[string]$Step='finish P7',[string]$NextAction='enter P8') {
  ...
  $hostRoot = Join-Path $TestDrive ('h-' + $token)
  ...
  $proofInput = New-PhaseCloseoutProof $hostRoot $Phase $Step $NextAction
  $proofBase64 = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes(($proofInput | ConvertTo-Json -Depth 8 -Compress)))
  $created = Invoke-PhaseCloseoutContract $hostRoot @(
    ...
    '-ProjectRoot',$hostRoot,'-ProjectProgressProofBase64',$proofBase64,
  ...
  if ($ActivateH7) {
    ...
    $checkpoint = Invoke-PhaseCloseoutH7 $stateRoot $hostRoot $sessionKey 'checkpoint' $checkpointBase64
    if ($checkpoint.exitCode -ne 0 -or $checkpoint.value.checkpoint.ok -ne $true) { throw 'phase closeout H7 checkpoint setup failed' }
    $current = Invoke-PhaseCloseoutContract $hostRoot @('-Action','Get','-TaskId',$taskId,'-WorkspaceKey',$workspaceKey,'-SessionKey',$sessionKey,'-StateRoot',$stateRoot,'-Json')
    if ($current.exitCode -ne 0) { throw 'phase closeout H7 current contract read failed' }
    $contract = $current.value
    $evidence = Invoke-PhaseCloseoutH7 $stateRoot $hostRoot $sessionKey 'evidence'
    if ($evidence.exitCode -ne 0) { throw 'phase closeout H7 evidence setup failed' }
  }
  return [pscustomobject]@{
    stateRoot=$stateRoot;hostRoot=$hostRoot;workspaceKey=$workspaceKey;sessionKey=$sessionKey;taskId=$taskId
