...
function Invoke-WorkspaceContract([string[]]$Arguments,[string]$StateRoot) {
  $boundArguments = @($Arguments) + @('-StateRoot',$StateRoot,'-NoExit','-Json')
  return (Invoke-H7FixtureContractScript -ContractScript $Contract -Root $Root -Arguments $boundArguments).value
}
...
Describe 'Workspace-scoped continuation consumers' {
  ...
  It 'excludes foreign task graph ledger and continuity data from snapshots' {
    ...
    $scopedContext = [pscustomobject]@{status='active';stale=$false;taskId=$taskId;workspaceKey=$workspaceA;acceptedGoal='exact workspace goal';expiresAt=(Get-Date).AddHours(1).ToString('o')}
    $pointerPath = Get-SuperBrainCanonicalTaskPath (Join-Path $workspace 'guard-state\current-task-context-pointers') $workspaceA '.json'
    Write-WorkspaceTestJson $pointerPath $scopedContext
    Write-WorkspaceTestJson (Join-Path $workspace 'current-task-context.json') ([pscustomobject]@{status='active';stale=$false;taskId='task-foreign-context';workspaceKey=$workspaceB;acceptedGoal=$foreignMarker;expiresAt=(Get-Date).AddHours(1).ToString('o')})
  ...
  It 'uses the bound contract rather than a stale same-task checkpoint for snapshot progress' {
    $stateRoot = Join-Path $TestDrive 'contract-overrides-checkpoint'
    $workspace = Join-Path $stateRoot 'workspace'
    $checkpointRoot = Join-Path $workspace 'runtime-state\checkpoints\active'
    $taskId = 'task-contract-snapshot'
    $workspaceKey = 'ws-a88888888888888888888888'
    $completed = @(1..8 | ForEach-Object { 'contract complete ' + $_ })
    $contractArguments = @('-Action','Set','-TaskId',$taskId,'-WorkspaceKey',$workspaceKey,'-FocusId','main-line','-LatestUserInstruction','approve the canonical contract plan','-NextAction','contract next action','-CurrentPhase','contract phase','-CurrentStep','contract current step','-CompletedSteps') + $completed + @('-PendingSteps','contract pending one','contract pending two','-Blockers','contract blocker','-EnableCanonicalPlan')
    $contract = Invoke-WorkspaceContract -Arguments $contractArguments -StateRoot $stateRoot
    $contract.ok | Should Be $true

    Write-WorkspaceTestJson (Join-Path $checkpointRoot ($taskId + '.json')) ([pscustomobject]@{
      taskId=$taskId; workspaceKey=$workspaceKey; status='active'; currentPhase='STALE_CHECKPOINT_PHASE'; currentStep='STALE_CHECKPOINT_STEP'; pendingSteps=@('stale one','stale two','stale three'); completedSteps=@(); blockers=@()
    })

    $snapshot = Invoke-WorkspaceJsonScript $SnapshotWriter @('-WorkspaceKey',$workspaceKey,'-AllowActiveCheckpoint','-Json') $stateRoot
    $snapshot.exitCode | Should Be 0
    $snapshot.value.continuity.source | Should Be 'execution-contract.ps1'
    $snapshot.value.continuity.currentPhase | Should Be 'contract phase'
    $snapshot.value.continuity.currentStep | Should Be 'contract current step'
    $snapshot.value.continuity.openStepCount | Should Be 2
    $snapshot.value.continuity.completedCount | Should Be 8
    $snapshot.value.continuity.blockedCount | Should Be 1
    $snapshot.text.Contains('STALE_CHECKPOINT_PHASE') | Should Be $false
  }
  ...
  It 'restores a parent-return plan while keeping the packet bounded' {
    ...
    $main = Invoke-WorkspaceContract -Arguments @('-Action','Set','-TaskId',$taskId,'-WorkspaceKey',$workspaceKey,'-FocusId','main-line','-NextAction','resume exact main action') -StateRoot $stateRoot
    $main.ok | Should Be $true
    $side = Invoke-WorkspaceContract -Arguments @('-Action','Set','-TaskId',$taskId,'-WorkspaceKey',$workspaceKey,'-InstructionMode','side_branch','-FocusId','side-line','-NextAction','finish side line') -StateRoot $stateRoot
    $side.ok | Should Be $true
    $resumed = Invoke-WorkspaceContract -Arguments @('-Action','ResumeParent','-TaskId',$taskId,'-WorkspaceKey',$workspaceKey,'-BranchStatus','completed','-CompletionEvidence','side verified') -StateRoot $stateRoot
    $resumed.ok | Should Be $true
    $resumed.focusId | Should Be 'main-line'
    ...
    $restore.exitCode | Should Be 0
    # The latest durable assistant progress receipt now takes precedence over
    # the contract projection while preserving the same bound parent return.
    $restore.value.recoveryPoint.source | Should Be 'latest_assistant_progress_receipt'
