Describe 'Execution-state resume regression guards' {
  ...
  It 'requires an explicit resume receipt with last-sentence evidence' {
    ...
    foreach ($marker in @('## Resume Receipt','Do not claim durable progress from vague memory','before mutation','Presentation correctness is part of the receipt contract','External prose never becomes','historical','When `actionAuthorization=withheld`','clearly labeled checkpoint-only state')) { $recovery.Contains($marker) | Should Be $true }
    $openingLine = ([char]0x672c)+([char]0x5730)+([char]0x6267)+([char]0x884c)+([char]0x5951)+([char]0x7ea6)+([char]0xff1a)+([char]0x8fdb)+([char]0x5ea6)+([char]0xff1a)
    $nextWord = ([char]0x4e0b)+([char]0x4e00)+([char]0x6b65)
    foreach ($marker in @($openingLine,'current local progress receipt','H7-PROGRESS-V4 receipt_hash',$nextWord)) { $recovery.Contains($marker) | Should Be $true }
  }
  ...
  It 'keeps skill capability map for ORC skill synergy' {
    ...
    foreach ($marker in @('super-brain.skill-capability-map.result.v1','category','role','triggers','applyAt','verification','IncludeAuditHints','cannotDo','stopCondition','extension-capability-map.ps1','absorbed-capability-map.json','skill-capability-map.seed.json','package_public_seed','capabilityMapOrigin','capabilityMapRehydrated','List','Detail','NoExtensions','absorbed capability','do not require standalone host skills or force the user to remember source names')) { $scriptText.Contains($marker) | Should Be $true }
  ...
  It 'keeps extension ingest and capability routing visible through brain commands' {
    ...
    foreach ($marker in @('super-brain.absorbed-capability-map.v1','Get-VerifiedExtensionManifests','setupRequired','provenance','absorbed-capability-map.ps1')) { $extensionMapText.Contains($marker) | Should Be $true }
    $ingestText = Get-Content -LiteralPath (Join-Path $root 'scripts\extension-ingest.ps1') -Raw -Encoding UTF8
    foreach ($marker in @("ValidateSet('List','Inspect','Adopt','RebuildMap')",'Inspect-Path','extension.json','.claude-plugin\plugin.json','SKILL.md','Get-InstalledState','suggestedAction','extensionCapabilityMap','Run verify-extensions, skill-capability-map, and verify-package')) { $ingestText.Contains($marker) | Should Be $true }
...
Describe '0.5.28 regression guards' {
  ...
  It 'restores memory sharing policy after smoke tests' { $scriptText = Get-Content -LiteralPath (Join-Path $root 'scripts\smoke-test.ps1') -Raw -Encoding UTF8; $scriptText.Contains('$savedEnvironment') | Should Be $true; $scriptText.Contains('SUPER_BRAIN_STATE_ROOT') | Should Be $true; $scriptText.Contains('restore them in ``finally``') | Should Be $true; $scriptText.Contains('Remove-Item -LiteralPath $tmpRoot -Recurse -Force') | Should Be $true }
...
Describe 'Root marker and startup bootstrap guards' {
  ...
  It 'keeps one thin canonical startup block that delegates policy to Super Brain' {
    ...
    foreach ($marker in @('legacyPattern','Super Memory Brain Bootstrap','Entry: explicit Super Brain/G1','Git workflow trigger','Authority: bootstrap only','super-brain-rules.json','must never duplicate or override','same H7 CLI','Never use Hook/P7','PACKAGE_ROOT_MARKER_SOURCE_MISSING','PACKAGE_ROOT_MARKER_VERIFY_FAILED')) {
