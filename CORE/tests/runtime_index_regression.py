...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="super-brain-index-regression-") as directory:
        ...
        try:
            lock_module = importlib.import_module("sandglass_lock")
            retry_target = str(memory_root / "permission-retry")
            retry_lock_path = retry_target + ".lock"
            original_open = lock_module.os.open
            transient_failures = {"count": 0}

            def windows_contention_open(path: str, flags: int, mode: int = 0o777) -> int:
                if path == retry_lock_path and transient_failures["count"] == 0:
                    transient_failures["count"] += 1
                    raise PermissionError(13, "simulated Windows lock contention", path)
                return original_open(path, flags, mode)

            lock_module.os.open = windows_contention_open
            try:
                retry_lock = lock_module.FileLock(retry_target, timeout=1.0, poll=0.001)
                descriptor = retry_lock.acquire()
                retry_lock.release(descriptor)
            finally:
                lock_module.os.open = original_open
            assert transient_failures["count"] == 1
        finally:
