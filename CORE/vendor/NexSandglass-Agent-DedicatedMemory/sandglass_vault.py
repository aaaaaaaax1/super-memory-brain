...
def _write_idx(idx):
    ...
    directory = os.path.dirname(os.path.abspath(_IDX))
    os.makedirs(directory, exist_ok=True)
    fd = -1
    tmp = ""
    try:
        with locked_file(_IDX, timeout=15.0, stale_after=120.0):
            fd, tmp = tempfile.mkstemp(
                prefix=os.path.basename(_IDX) + ".",
                suffix=".tmp",
                dir=directory,
                text=True,
            )
            ...
            for attempt in range(7):
                try:
                    ...
                    if attempt == 6:
                        raise
                    time.sleep(min(0.16, 0.01 * (2**attempt)))
            tmp = ""
    except MemoryLockTimeout:
        logger.warning("sandglass: index write lock timed out")
        raise
    finally:
        if fd >= 0:
            try:
...
def idx_search(query: str, limit: int = 100) -> list:
    ...
    try:
        ...
        if not idx:
            return []
        # The index is built with this module's tokenizer; importing the full
        # thinking stack here adds avoidable cold-start latency and side effects.
...
def _read_idx() -> tuple[dict, int]:
    """Read one complete index generation without racing an atomic replace."""
    idx = {}
    idx_max = 0
    with locked_file(_IDX, timeout=15.0, stale_after=120.0):
        ...
        with open(_IDX, "r", encoding="utf-8") as f:
            for line in f:
                if line.startswith("#") or ":" not in line:
                    continue
                token, rest = line.strip().split(":", 1)
                nums = [int(value) for value in rest.split(",") if value]
                idx[token] = nums
                if nums:
